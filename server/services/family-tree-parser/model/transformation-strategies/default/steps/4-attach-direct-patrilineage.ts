...
export default declareTransformationStep(
  ...
  (input: Input) => {
    for (const n of input.nodes.persons.all) {
      ...
      traverseGraphDepthFirst(
        ...
        (node) => {
          if (node.type === "Marriage") {
            ...
            const progenitorIdentity = input.adjacencies.byPerson.single
              .progenitor.get(node.identity);
            const adoptiveProgenitorIdentity = input.adjacencies.byPerson.single
              .adoptiveProgenitor.get(node.identity);
            return [progenitorIdentity, adoptiveProgenitorIdentity].map((n) =>
              input.nodes.marriages.$one(n)
            ).filter((n) => !!n);
          }
        },
        {
          visit(context) {
            if (context.currentNode.node.type === "Marriage") {
              ...
              // add mother node
              const motherIdentity = input.adjacencies.byMarriage.single.bride
                .get(context.currentNode.node.identity);
              ...
              // add links
              const lProgeny = input.links.$one({
                type: "MaritalProgeny",
                toNodeIdentity: context.currentNode.node.identity,
              });
              const lAdoptedProgeny = input.links.$one({
      ...
      n.generationInClan = n.patrilineage.nodes.filter((n) =>
        n.startsWith("male")
      ).length + 1;
      const rootAncestor = (() => {
        const tmp = n.patrilineage?.nodes?.[0];
        if (tmp && tmp.startsWith("male:")) {
          return input.nodes.persons.male.$one(tmp as MaleIdentity) ??
            undefined;
        }
      })();
