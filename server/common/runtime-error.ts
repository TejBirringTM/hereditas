 ...
 */
export function declareRuntimeError(name: string) {
    return Object.freeze({
        create: (message?: string) => new RuntimeError(name, message),
        validate: (error?: {name?: unknown}) => error?.name && typeof error.name === "string" && error.name ===  name
    })
}
