...
const ApiRequestErrors = {
  requestBodyNotJsonError: declareErrorResponse(
    Status.BadRequest,
  ...
  >(
    issues: [
      InferIssue<StrictObjectSchema<TRequestEntries, TRequestErrorMessage>>,
      ...InferIssue<
        StrictObjectSchema<TRequestEntries, TRequestErrorMessage>
      >[],
    ],
  ) =>
    declareErrorResponse(
      Status.BadRequest,
      "The request body is a valid JSON object but it does NOT match the schema.",
      issues,
    ),
} as const;
