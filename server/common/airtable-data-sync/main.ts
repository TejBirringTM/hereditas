...
> {
    ...
    private readonly transform : (input: v.InferOutput<typeof this.schema>) => TOut;
    private _records : TOut[];
    private lastFetchTimestamp : number;
    
    constructor(baseId: string, tableId: string, options: TableSyncOptions, schema: v.ObjectSchema<TEntries, TMessage>, transform?: (input: v.InferOutput<v.ObjectSchema<TEntries, TMessage>>) => TOut) {
        this.baseId = baseId;
    ...
    private async fetch() {
        ...
        const records = await airtableRecords(this.baseId, this.tableId, this.selectOptions, this.schema);
        this._records = [];
