...
const validationErrors = {
    ValidationFailed: (name: string, reasons: string[]) => declareRuntimeError(`validation failed, invalid ${name}: ${reasons.join(', ')}`)
} as const;

export function declareString<TName extends string>(name: TName, formatDescription: string, formatRegex: RegExp) {
    const schema = v.pipe(
        v.string(`${name} must be a string`),
        v.regex(formatRegex, `${name} is of incorrect format:\n${formatDescription}`),
    );
...
>(name: TName, schema: TSchema) {
    return function(input: unknown) {
        ...
        if (result.success) {
            ...
            throw validationErrors.ValidationFailed(name, result.issues.map((issue)=>issue.message));
        }
    }
}