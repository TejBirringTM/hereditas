...
export const promptV1 = makePromptsTemplate(({grammar, familyTreeDescription}: {grammar: string, familyTreeDescription: string}) => {
...
`
    ...
    const instructionalSystemPrompt : Prompt = {
        role: "system",
        content: "For each input, provide only the EBNF output with no additional explanation. If this is not possible, just respond with \"FAILED: \" followed by a brief reason why."
    }

    const userPrompt : Prompt = {
        role: "user",
        content: 
`Here is a description of a family tree in plain English:
